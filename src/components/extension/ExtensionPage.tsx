...
const LAYOUT_EXAMPLES: Record<string, string> = {
  ...
  'stat-row': '[{"label":"覆盖模块数","value":"43 个","trend":"+12%","color":"accent"},{"label":"支持主题","value":"48 套","trend":"+8 套","color":"default"},{"label":"平均导出耗时","value":"0.8s","trend":"-40%","color":"accent"},{"label":"月活创作者","value":"12K+","trend":"+23%","color":"default"}]',
  ...
  'comparison-table': '{"headers":["对比维度","传统手工排版","MarkFlow 模块化"],"rows":[["上手时间","2-4 周","10 分钟"],["品牌一致性","依赖设计师手感","48 套主题自动保证"],["移动端适配","需逐篇调整","所有模块内置适配"],["导出格式","公众号富文本","富文本 + 长图 + A4 文档 + 卡片"],["协作难度","高（样式易丢失）","低（Markdown 纯文本）"]]}',
  changelog: '{"version":"v2.4.0","date":"2026-07-01","added":["组件属性编辑器：光标定位到组件标签时自动弹出属性面板","轮播图支持淡入淡出模式"],"changed":["主题预览面板合并为「主题与配色」统一入口","Layout 模块 ID 增加 layout- 前缀避免冲突"],"fixed":["PC 端页面宽度异常（Paged.js polyfill 冲突）","组件库分类全部 63 个组件正确映射到 11 个功能分组"]}',
};
...
const componentCategoryMap: Record<string, string> = {
  ...
  'layout-quote-card': 'emph', 'layout-tweet': 'emph',
  'layout-question': 'content', 'layout-faq': 'content',
  'layout-summary': 'content', 'layout-notice': 'content', 'layout-definition': 'content',
  'layout-cases': 'cards', 'layout-author-card': 'cards', 'layout-subscribe': 'cards',
  'layout-people': 'cards', 'layout-series': 'cards', 'layout-resource-list': 'cards',
...
export function ExtensionPage({ onClose }: ExtensionPageProps) {
  ...
  useEffect(() => {
    ...
    setExamples([...tagExamples, ...unifiedExamples, ...layoutExamples]);
  }, [colors]);
    ...
    <div ref={cardRef} className="group relative break-inside-avoid mb-5 inline-block w-full rounded-2xl bg-white border border-slate-200 shadow-sm cursor-default overflow-hidden" style={{ '--mouse-x': '50%', '--mouse-y': '50%' } as React.CSSProperties} onMouseMove={handleMouseMove}>
      ...
      <div className="relative z-[1]">
        ...
        <div className="px-5 pb-4">
          {rendered ? (
            <div className="preview-content max-w-full [&_section]:transition-opacity [&_section]:duration-150" dangerouslySetInnerHTML={{ __html: rendered }} />
          ) : (
