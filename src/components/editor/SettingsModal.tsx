            ...
            <div className="rounded-lg bg-slate-50 p-4 border border-slate-100 flex flex-col gap-3">
              ...
              <div className="grid grid-cols-1 gap-2.5">
                <div className="flex flex-col gap-1">
                  ...
                  <Input value={wechatForm.appId} onChange={(e) => setWeChatForm((v) => ({ ...v, appId: e.target.value }))} placeholder="wx..." />
                </div>
                <div className="flex flex-col gap-1">
                  <label className="text-[12px] font-medium text-slate-600">AppSecret</label>
                  <Input type="password" value={wechatForm.appSecret} onChange={(e) => setWeChatForm((v) => ({ ...v, appSecret: e.target.value }))} placeholder="仅存当前会话内存" />
                </div>
                <div className="flex flex-col gap-1">
                  <label className="text-[12px] font-medium text-slate-600">封面素材 thumb_media_id（可选）</label>
                  <Input value={wechatForm.thumbMediaId} onChange={(e) => setWeChatForm((v) => ({ ...v, thumbMediaId: e.target.value }))} placeholder="留空则自动上传封面图" />
                </div>
                <div className="flex flex-col gap-1">
                  <label className="text-[12px] font-medium text-slate-600">封面图 URL（可选）</label>
                  <Input value={wechatForm.coverImageUrl} onChange={(e) => setWeChatForm((v) => ({ ...v, coverImageUrl: e.target.value }))} placeholder="https://.../cover.jpg；留空则使用正文第一张图片" />
                </div>
                <div className="flex flex-col gap-1">
                  <label className="text-[12px] font-medium text-slate-600">发布服务端点</label>
                  <Input value={wechatForm.publishEndpoint} onChange={(e) => setWeChatForm((v) => ({ ...v, publishEndpoint: e.target.value }))} className="font-mono text-[12px]" />
                </div>
              </div>
            </div>
          ) : activeTab === 'ai' ? (
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
                  {(
                    [
                      ...
                      { type: 'catbox', name: 'Catbox 免费图床' },
                      { type: 'smms', name: 'Sm.ms（已收费）' },
