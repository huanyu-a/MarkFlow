...
- ^文字^          上标（如 m^2^）
- \`文字\`          行内代码
- ${bt('<Badge type="info" text="标签" />')}  行内徽章（type 可选 info/tip/warning/danger）
- ${bt('<Icon name="material-symbols:star" />')}  行内图标`;
...
1. 只能使用上面列出的语法与标签，不要发明新标签或新属性。不要直接混入 ${bt('<script>')}、事件处理器属性（如 \`onclick\`）、\`javascript:\` 链接或未列出的任意 HTML 标签，以免造成渲染异常或安全风险。
...
10. 表格后必须空一行：表格（含 ${bt(':::table')} 容器）结束后必须空一行再写下一段正文，否则紧随其后的非表格行会被系统当作表格注释吞掉（渲染为小字灰色文本）。
11. ${bt(':::table')} 可选属性（均可省略）：style 选风格（default/striped/card）；title 写表格标题；caption 与 title 等价，两者同时写以 title 为准；footer 写表注脚，会覆盖闭合后紧邻的隐式注脚。标题优先级为 title > caption > ${bt(':::table')} 后的位置文字。最小示例：
   \`\`\`
   :::table style="card" title="四种输出模式对比" footer="数据来源：MarkFlow 使用统计"
   | 输出方式 | 适合场景 |
   |----------|----------|
   | 复制富文本 | 公众号 |
   :::
...
1. **不要使用**长图文里的社交互动组件（如 ${bt('<breaking>')}、${bt('<timeline>')}、${bt('<engage>')} 等），保持文档正式、严谨、适合打印和归档。不要直接混入 ${bt('<script>')}、事件处理器属性（如 \`onclick\`）、\`javascript:\` 链接或未列出的任意 HTML 标签，以免造成渲染异常或安全风险。
2. 表格首行（表头）内容默认会强制居中，表格体内容默认左对齐。**表格（含 ${bt(':::table')} 容器）结束后必须空一行再写下一段正文**，否则紧随其后的非表格行会被系统当作表格注释吞掉（小字灰色）。${bt(':::table')} 可选属性：style（default/striped/card）、title 或 caption（表格标题，两者同写以 title 为准）、footer（表注脚，覆盖闭合后紧邻行）。
...
请使用以下元数据生成封面页，表格后必须插入 ${bt('<page-break/>')} 再开始正文：
...
- 表格后必须插入 ${bt('<page-break/>')} 分页符，再开始正文。`
}
...
function buildGovDocMetadataSection(meta: GovDocMetadata): string {
  ...
  lines.push('')
  lines.push(`请使用以下元数据生成 ${bt('<gov-header>')} 标签：`)
  lines.push('')
  lines.push('```')
  ...
  lines.push('要求：')
  lines.push(`- 上述元数据已由用户确认，请原样填入 ${bt('<gov-header>')} 标签属性，不要修改、补全或省略已填写的字段。`)
...
export function buildArticleAiGuide(): string {
  return [
    ...
    `5. 结尾给出清晰总结或行动提示，推荐使用 ${bt('<engage type="DA02">')}（彩色引导卡片样式）引导收藏、关注或分享。`,
    "",
  ].join("\n");
}
...
const GOV_DOC_HEADER_SECTION = `## 三、公文头部写法（${bt('<gov-header>')} 标签）

公文必须在正文开始前使用 ${bt('<gov-header>')} 标签渲染红头文件头部。标签属性：
...
1. **必须使用 ${bt('<gov-header>')} 标签**渲染公文头部，不要用普通标题或表格模拟红头文件。
2. 公文标题（正文第一个一级标题）居中显示，使用二号宋体加粗。
3. 主送机关左顶格，后跟全角冒号，如"各区人民政府，市政府各委、办、局："
4. 正文使用仿宋字体，首行缩进两字，两端对齐。
5. 落款（发文机关署名 + 日期）右对齐，距正文两行。
6. **不要使用**长图文里的社交互动组件（如 ${bt('<breaking>')}、${bt('<timeline>')}、${bt('<engage>')} 等），保持公文严肃性。不要直接混入 ${bt('<script>')}、事件处理器属性（如 \`onclick\`）、\`javascript:\` 链接或未列出的任意 HTML 标签。
7. 附录前必须插入 ${bt('<page-break/>')}。
...
技术文档可选择是否需要封面页。如需封面，在第一个 ${bt('<page-break/>')} 之前只写一个一级标题和一个信息表格。
...
- 不需要封面时，直接从正文开始，不要插入 ${bt('<page-break/>')}。`

const TECH_DOC_NO_COVER_SECTION = `## 三、封面页写法（技术文档）

**本文档不需要封面页。** 请直接从正文开始，不要在开头插入 ${bt('<page-break/>')}。
...
export function buildCardAiGuide(aspect: string): string {
  ...
  return [
    ...
    `- 不要直接输出 HTML 样式或混入 ${bt('<script>')}、事件处理器属性（如 \`onclick\`）、\`javascript:\` 链接；需要强调时使用 ==重点==、!!标签!!、^^强强调^^ 等行内语法。`,
