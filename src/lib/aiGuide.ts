...
10. 表格后必须空一行：表格（含 ${bt(':::table')} 容器）结束后必须空一行再写下一段正文，否则紧随其后的非表格行会被系统当作表格注释吞掉（渲染为右对齐的小字灰色文本）。`;
...
2. 表格首行（表头）内容默认会强制居中，表格体内容默认左对齐。**表格（含 ${bt(':::table')} 容器）结束后必须空一行再写下一段正文**，否则紧随其后的非表格行会被系统当作表格注释吞掉（小字灰色）。
