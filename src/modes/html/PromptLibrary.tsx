...
import { buildDesignPrompt, buildCustomDesignPrompt } from '@/data/designPrompts'
...
export function PromptLibrary({ mode, open, onClose, onCopy, onToast }: PromptLibraryProps) {
  ...
  return (
    <>
      ...
      />
      <aside
        className={`fixed left-1/2 top-1/2 z-50 flex w-[90vw] max-w-[1000px] max-h-[85vh] -translate-x-1/2 -translate-y-1/2 flex-col bg-slate-50 shadow-2xl transition-all duration-200 ease-out rounded-xl overflow-hidden ${
                          ...
                          >
                            ...
                            <div className="mt-auto flex gap-2">
                              <button
                                ...
                                className="accent-bg flex flex-1 items-center justify-center gap-1.5 rounded-lg py-2.5 text-[13px] font-semibold text-white opacity-90 transition-all hover:opacity-100 hover:shadow-md hover:ring-2 hover:ring-[var(--accent)] hover:ring-offset-2 active:scale-[0.98] active:opacity-80 shadow-sm"
                              >
                                <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><rect x="9" y="9" width="13" height="13" rx="2" ry="2"></rect><path d="M5 15H4a2 2 0 0 1-2-2V4a2 2 0 0 1 2-2h9a2 2 0 0 1 2 2v1"></path></svg>
                                {UI_LABELS.promptLibrary.copyPrompt.label}
                              </button>
                              <button
                                onClick={() => setPreviewText(buildDesignPrompt(getCustomizedStyle(s)))}
                                className="flex items-center justify-center rounded-lg border border-slate-200 px-2.5 py-2 text-[13px] text-slate-500 hover:bg-slate-50 hover:text-slate-700 transition-colors"
                                title="预览指令"
                              >
                    ...
                    {filteredCustomInstructions.map((inst) => (
                      ...
                      >
                        ...
                        <div className="mt-auto flex gap-2">
                          ...
                          <button
                            onClick={() => setPreviewText(buildCustomDesignPrompt(inst.content))}
                            className="flex items-center justify-center rounded-lg border border-slate-200 px-2.5 py-2 text-[13px] text-slate-500 hover:bg-slate-50 hover:text-slate-700 transition-colors"
                            title="预览指令"
                          >
                            <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z"/><circle cx="12" cy="12" r="3"/></svg>
              ...
              <div className="flex items-center gap-2">
                <button
                  onClick={async () => {
                    ...
                    if (ok && onToast) onToast('已复制指令')
                  }}
                  className="flex items-center gap-1.5 rounded-md bg-slate-900 px-3 py-1.5 text-[12px] font-medium text-white hover:bg-slate-800 transition-colors"
                >
