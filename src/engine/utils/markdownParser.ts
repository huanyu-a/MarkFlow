...
export function parseMarkdown(
  ...
  if (stdFootnotes.length > 0) {
    processedMd = processedMd.replace(/^\[\^(\w+)\]:\s+.+$/gm, '')
  }
  // Replace footnote references [^label] with superscript markers
  processedMd = processedMd.replace(/\[\^(\w+)\](?!:)/g, (_match, label) => {
    const idx = stdFootnotes.findIndex(f => f.label === label)
    if (idx >= 0) {
      return `<sup><a href="#fn-${label}" id="fnref-${label}" style="color:${t.accent};text-decoration:none;font-weight:600">[${idx + 1}]</a></sup>`
    }
    return _match
  })

  // 收集行内脚注：^[行内脚注文本]
  let inlineFnCounter = stdFootnotes.length
  processedMd = processedMd.replace(/\^\[([^\]]+)\]/g, (_match, text) => {
    inlineFnCounter++
    const label = `inline-${inlineFnCounter}`
    stdFootnotes.push({ label, definition: text.trim() })
    return `<sup><a href="#fn-${label}" id="fnref-${label}" style="color:${t.accent};text-decoration:none;font-weight:600">[${inlineFnCounter}]</a></sup>`
  })
  ...
  if (footnotes.length > 0 || stdFootnotes.length > 0) {
    ...
    stdFootnotes.forEach((fn, idx) => {
      const num = footnotes.length + idx + 1
      fnParts.push(`<p id="fn-${fn.label}" style="margin:${spacing[2]} 0px"><span style="color:${t.accent};font-weight:${fontWeight.semibold}">[${num}]</span> ${leaf(fn.definition)} <a href="#fnref-${fn.label}" style="color:${t.accent};text-decoration:none;font-size:${fontSize.sm}">↩</a></p>`)
    })
