...
//   - 内容：非贪婪、首尾非空白、内部不含 $（捕获组 [^$]+? 硬约束，拒绝跨配对
//     误合并，如「$a$$b$」不会回溯合并成一个坏公式送 KaTeX；「价格从$5涨到$6」
//     也不会把中间内容吞成公式）
//   - 结束 $：后面不接 $$（避免吃掉块级定界符开头）、不接字母数字/下划线
//     （「$6.99」式价格尾部、「$x$5」量价混排不算公式）
// 注意：行内 $ 允许跨行（与块级 $$ 的多行写法保持对称），行尾双空格换行等
// Markdown 细节不受影响（$ 本身不是 Markdown 控制字符）。
// 已知取舍：内容不含 $ 意味着公式内若需字面美元符（如 KaTeX 的 $\$5$）无法被
// 行内语法识别——此类需求应改用块级 $$ 或文字描述，属极罕见场景。
export const INLINE_MATH_SOURCE = String.raw`(?<![\d$])\$(?![\s\d$])([^$]+?)(?<!\s)\$(?!\$|\w)`
// 块级 $$...$$（允许跨行，非贪婪配对）
