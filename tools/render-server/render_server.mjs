...
// 预览页模板（含 KaTeX 公式样式内嵌）见 preview.mjs，与 smoke-test 共用
...
const server = createServer(async (req, res) => {
  ...
  try {
    ...
    const { html, meta } = renderMarkdown(
      markdown,
      makeColors(accent, dark),
      undefined,
      (w) => warnings.push(String(w)),
    )

    const metaOut = { title: meta.title, summary: meta.summary }
    if (warnings.length > 0) metaOut.warnings = warnings
