...
按 `guide` 把用户素材整理成 Markdown（保留用户原意，不虚构事实），然后：
...
返回 `{"ok":true,"html":"<h1 ...>...</h1>","meta":{"title":"...","summary":"..."}}`。`html` 为全内联样式的片段（可直接贴入公众号编辑器或存为 `.html` 文件交付）。
...
| `accent` | 可选 | 主题主色，6 位 hex（如 `#e74c3c`）；默认 `#27ae60` |
| `dark` | 可选 | 主题深色，6 位 hex；默认 `#1e8449` |

错误码：401 token 错误；400 JSON 非法或 markdown 为空；413 超过 2MB；500 渲染失败（返回 `error` 信息）。

## 主题色参考

不带参数即默认绿色系（与 MarkFlow 前端默认一致）。常见搭配：科技蓝 `#0984e3`/`#0769b5`、商务红 `#e74c3c`/`#c0392b`、墨绿 `#556B2F`/`#3d4f1f`、深藏蓝 `#1e3a5f`/`#0f2744`。色彩搭配拿不准时优先用默认值。
